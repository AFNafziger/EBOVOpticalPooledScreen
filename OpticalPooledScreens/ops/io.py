...
def format_input(input_table, n_jobs=1, channel_order=None, **kwargs):
    ...
    **kwargs
    ...
    def process_site(output_file,df_input):
        stacked = np.array([read_nd2_stack(input_file) for input_file in
    ...
    if n_jobs != 1:
        ...
        Parallel(n_jobs=n_jobs, **kwargs)(delayed(process_site)(output_file,df_input)
            for output_file,df_input in df.groupby('wsnakemake filename'))
    else:
...
def format_input_nd2(input_table, n_jobs=1, channel_order=None, **kwargs):
    """Formats filenames and concatenates images from the same field of view
    and cycle of imaging if necessary (e.g., when individual channels are split
    between tiff file outputs of microscope control software). See example
    input_file.xlsx table. Also saves `input/well_tile_list.csv` to use as input
    ...
    **kwargs
    ...
    if channel_order is None:
        channel_order = {'ALL':0,'DAPI':1,'G':2,'T':3,'A':4,'C':5}
        #channel_order = {'ChannelBV421_10p_30ms_STING':0,'ChannelGFP_30p_STING':1,'ChannelA594_80p_400ms_STING':2,'ChannelCY5_30p_STING':3}
    cycle = None
    if cycle is None:
        print('no cycles')
        df['cycle'] = 0
    ...
    df['channel_order'] = df['channel'].map(channel_order)
    #print(df.tolist())
    #print(pd.unique(df['snakemake filename']))
    #print(df['snakemake filename'].tolist())
    ...
    def process_site(output_file,df_input):
        ...
        #stacked = np.concatenate((stacked[0],stacked[1]))
        save_stack(output_file,stacked)
        #print(stacked.shape)
    if n_jobs != 1:
        from joblib import Parallel, delayed
...
if __name__ == '__main__':
    commands = {
     ...
     'format_input_nd2': format_input_nd2}
    try:
        fire.Fire(commands)
    except QuitError:
        sys.exit(1)
